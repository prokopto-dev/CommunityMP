...
Player = require("communitymp.saves.playerAccountStore")
Cell = require("communitymp.saves.cellStateStore")
RecordStore = require("communitymp.saves.recordStateStore")
World = require("communitymp.saves.worldStateStore")
local serverStateRegistry = require("communitympServerStateRegistry")

function LoadBanList()
    local serverStatePaths = serverStateRegistry.getPaths()
    tes3mp.LogMessage(enumerations.log.INFO, "Reading " .. serverStatePaths.banListPath)
    banList = serverStateRegistry.loadBanList()
...
function OnServerInit()
    ...
    local expectedVersionPrefix = "0.1.0"
...
function LoadDataFileList(filename)
    local serverStatePaths = serverStateRegistry.getPaths()
    local sourceName = filename or serverStatePaths.dataFileRequirementsPath
    local dataFileList = {}
    tes3mp.LogMessage(enumerations.log.INFO, "Reading " .. sourceName)

    local requiredDataFiles = serverStateRegistry.loadDataFileRequirements(filename)

    if requiredDataFiles == nil then
        tes3mp.LogMessage(enumerations.log.ERROR, "Data file list " .. sourceName .. " cannot be read!")
        tes3mp.StopServer(2)
    else
        -- Fix numerical keys to print plugins in the correct order
        tableHelper.fixNumericalKeys(requiredDataFiles, true)

        for listIndex, pluginEntry in ipairs(requiredDataFiles) do
...
function OnRequestDataFileList()
    ...
    local dataFileList = LoadDataFileList()
...
function OnPlayerStatsDynamic(pid)
    eventHandler.OnPlayerStatsDynamic(pid)
end
...
function OnPlayerDisposition(pid)
    tes3mp.LogMessage(enumerations.log.INFO, "Called \"OnPlayerDisposition\" for " .. logicHandler.GetChatName(pid))
    eventHandler.OnPlayerDisposition(pid)
end

function OnPlayerInput(pid)
    eventHandler.OnPlayerInput(pid)
end

function OnPlayerRest(pid)
    eventHandler.OnPlayerRest(pid)
end

function OnPlayerBaseInfo(pid)
    tes3mp.LogMessage(enumerations.log.INFO, "Called \"OnPlayerBaseInfo\" for " .. logicHandler.GetChatName(pid))
    eventHandler.OnPlayerBaseInfo(pid)
end

function OnPlayerCharClass(pid)
    tes3mp.LogMessage(enumerations.log.INFO, "Called \"OnPlayerCharClass\" for " .. logicHandler.GetChatName(pid))
    eventHandler.OnPlayerCharClass(pid)
end
...
function OnActorStatsDynamic(pid, cellDescription)
    tes3mp.LogMessage(enumerations.log.INFO, "Called \"OnActorStatsDynamic\" for " .. logicHandler.GetChatName(pid) ..
        " and cell " .. cellDescription)
    eventHandler.OnActorStatsDynamic(pid, cellDescription)
end
...
function OnActorTest(pid, cellDescription)
    eventHandler.OnActorTest(pid, cellDescription)
end
...
function OnDoorDestination(pid, cellDescription)
    tes3mp.LogMessage(enumerations.log.INFO, "Called \"OnDoorDestination\" for " .. logicHandler.GetChatName(pid) ..
        " and cell " .. cellDescription)
    eventHandler.OnDoorDestination(pid, cellDescription)
end
