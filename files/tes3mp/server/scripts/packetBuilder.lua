...
packetBuilder.AddPlayerSpellsActive = function(pid, spellsActive, action, maxChangesPerPacket, sendChanges)

    tes3mp.ClearSpellsActiveChanges(pid)
    local currentAction = action
    tes3mp.SetSpellsActiveChangesAction(pid, currentAction)
    ...
    for spellId, spellInstances in pairs(spellsActive) do
        for spellInstanceIndex, spellInstanceValues in pairs(spellInstances) do
            ...
            if maxChangesPerPacket ~= nil and pendingChanges >= maxChangesPerPacket then
                if type(sendChanges) == "function" then
                    sendChanges(pid)
                end

                tes3mp.ClearSpellsActiveChanges(pid)
                currentAction = enumerations.spellbook.ADD
                tes3mp.SetSpellsActiveChangesAction(pid, currentAction)
                pendingChanges = 0
            end

            if currentAction == enumerations.spellbook.SET or currentAction == enumerations.spellbook.ADD then
...
packetBuilder.AddPlayerJournalItem = function(pid, journalItem)

    if journalItem.type == enumerations.journal.ENTRY then
        local actorRefId = journalItem.actorRefId
        if actorRefId == nil or actorRefId == "" then actorRefId = "player" end

        if journalItem.timestamp ~= nil then
            tes3mp.AddJournalEntryWithTimestamp(pid, journalItem.quest, journalItem.index, actorRefId,
                journalItem.timestamp.daysPassed, journalItem.timestamp.month, journalItem.timestamp.day)
        else
            tes3mp.AddJournalEntry(pid, journalItem.quest, journalItem.index, actorRefId)
        end
...
packetBuilder.AddObjectSpawn = function(uniqueIndex, objectData)
    ...
    if objectData.summon ~= nil then
        ...
        if objectData.summon.summoner.playerName or objectData.summon.summoner.playerKey then
            local player = getLoggedInPlayerByStorageKey(objectData.summon.summoner.playerKey)
            if player == nil then
                player = logicHandler.GetLoggedInPlayerByName(objectData.summon.summoner.playerName)
            end
            if player == nil then
                return false
            end
...
packetBuilder.AddDoorDestination = function(uniqueIndex, objectData)
    ...
    local splitIndex = uniqueIndex:split("-")
    local hasTeleportDestination = objectData.teleportState == true and objectData.doorDestination ~= nil

    tes3mp.SetObjectRefNum(splitIndex[1])
    tes3mp.SetObjectMpNum(splitIndex[2])
    if objectData.refId ~= nil then tes3mp.SetObjectRefId(objectData.refId) end
    tes3mp.SetObjectDoorTeleportState(hasTeleportDestination)

    if hasTeleportDestination then
        tes3mp.SetObjectDoorDestinationCell(objectData.doorDestination.cell)
        tes3mp.SetObjectDoorDestinationPosition(objectData.doorDestination.posX, objectData.doorDestination.posY,
            objectData.doorDestination.posZ)
        tes3mp.SetObjectDoorDestinationRotation(objectData.doorDestination.rotX, objectData.doorDestination.rotZ)
    end
