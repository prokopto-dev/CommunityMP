...
local function getPlayerRecordLinkKey(player)
    if player == nil then
        return nil
    end

    if type(player.GetRecordLinkKey) == "function" then
        return player:GetRecordLinkKey()
    elseif type(player.GetCharacterStorageKey) == "function" then
        return player:GetCharacterStorageKey()
    elseif type(player.GetCellVisitKey) == "function" then
        return player:GetCellVisitKey()
    end

    return player.accountName
end

local function hasRecordLink(data, storeType, recordId)
    return data ~= nil and data.recordLinks ~= nil and data.recordLinks[storeType] ~= nil and
        tableHelper.containsValue(data.recordLinks[storeType], recordId)
end

local function playerHasRecordLink(player, storeType, recordId)
    if player == nil or player.data == nil then
        return false
    end

    if hasRecordLink(player.data, storeType, recordId) then
        return true
    end

    local selectedIndex = player.activeCharacterIndex

    if selectedIndex == nil and player.data.characters ~= nil then
        selectedIndex = player.data.characters.selectedIndex
    end

    if player.data.characters == nil or type(player.data.characters.entries) ~= "table" then
        return false
    end

    for characterIndex, characterData in pairs(player.data.characters.entries) do
        if characterIndex ~= selectedIndex and hasRecordLink(characterData, storeType, recordId) then
            return true
        end
    end
...
function BaseRecordStore:AddLinkToPlayer(recordId, player)
    ...
    local playerLinkKey = getPlayerRecordLinkKey(player)
    local recordLinks = self.data.recordLinks

    if recordLinks[recordId] == nil then recordLinks[recordId] = {} end
    if recordLinks[recordId].players == nil then recordLinks[recordId].players = {} end

    if playerLinkKey ~= nil and not tableHelper.containsValue(recordLinks[recordId].players, playerLinkKey) then
        table.insert(recordLinks[recordId].players, playerLinkKey)
    end
end
...
function BaseRecordStore:RemoveLinkToPlayer(recordId, player)
    ...
    local playerLinkKey = getPlayerRecordLinkKey(player)
    local legacyAccountName = nil
    if player ~= nil then legacyAccountName = player.accountName end
    local recordLinks = self.data.recordLinks
    ...
    if playerLinkKey ~= nil then
        linkIndex = tableHelper.getIndexByValue(recordLinks[recordId].players, playerLinkKey)
    end
    ...
    if legacyAccountName ~= nil and legacyAccountName ~= playerLinkKey and
        not playerHasRecordLink(player, self.storeType, recordId) then
        linkIndex = tableHelper.getIndexByValue(recordLinks[recordId].players, legacyAccountName)
    ...
    tableHelper.cleanNils(recordLinks[recordId].players)
