...
local getPlayerAllyKey = function(player)
    if player ~= nil and type(player.GetCharacterStorageKey) == "function" then
        return player:GetCharacterStorageKey()
    end

    return player.accountName
end

local containsPlayerReference = function(values, player)
    if values == nil or player == nil then
        return false
    end

    return tableHelper.containsValue(values, getPlayerAllyKey(player)) or
        tableHelper.containsValue(values, player.accountName) or
        tableHelper.containsValue(values, player.name)
end

local removePlayerReferences = function(values, player)
    if values == nil or player == nil then
        return
    end

    tableHelper.removeValue(values, getPlayerAllyKey(player))
    tableHelper.removeValue(values, player.accountName)
    tableHelper.removeValue(values, player.name)
    tableHelper.cleanNils(values)
end

local sendCellChatMessage = function(pid, cellDescription, message)
    local sentToSender = false

    if cellDescription ~= nil and logicHandler.IsCellLoaded(cellDescription) == true and
        LoadedCells[cellDescription].visitors ~= nil then

        for index, visitorPid in pairs(LoadedCells[cellDescription].visitors) do
            tes3mp.SendMessage(visitorPid, message, false)

            if tonumber(visitorPid) == tonumber(pid) then
                sentToSender = true
            end
        end
    end

    if sentToSender == false then
...
defaultCommands.ooc = function(pid, cmd)
    if cmd[2] == nil then
        tes3mp.SendMessage(pid, "You cannot send a blank message.\n", false)
        return
    end

    local message = color.White .. "[OOC] " .. logicHandler.GetChatName(pid) .. ": " ..
...
customCommandHooks.registerCommand("ooc", defaultCommands.ooc)
customCommandHooks.registerCommand("global", defaultCommands.ooc)
customCommandHooks.registerCommand("g", defaultCommands.ooc)
...
defaultCommands.inviteAlly = function(pid, cmd)
    ...
    elseif logicHandler.CheckPlayerValidity(pid, cmd[2]) then
        ...
        local playerAllyKey = getPlayerAllyKey(Players[pid])
        local targetAllyKey = getPlayerAllyKey(Players[targetPid])

        if containsPlayerReference(Players[pid].data.alliedPlayers, Players[targetPid]) then
            senderMessage = "You already have " .. logicHandler.GetChatName(targetPid) .. " as your ally\n"
        elseif containsPlayerReference(Players[pid].allyInvitesSent, Players[targetPid]) then
            senderMessage = "You have already invited " .. logicHandler.GetChatName(targetPid) .. " to be your ally.\n"
        else
            table.insert(Players[pid].allyInvitesSent, targetAllyKey)
            table.insert(Players[targetPid].allyInvitesReceived, playerAllyKey)
...
defaultCommands.joinTeam = function(pid, cmd)
    ...
    elseif logicHandler.CheckPlayerValidity(pid, cmd[2]) then
        ...
        elseif containsPlayerReference(Players[pid].allyInvitesReceived, Players[targetPid]) then
            ...
            table.insert(Players[pid].data.alliedPlayers, getPlayerAllyKey(Players[targetPid]))
            table.insert(Players[targetPid].data.alliedPlayers, getPlayerAllyKey(Players[pid]))
            Players[pid]:Save()
            Players[pid]:LoadAllies()
            Players[targetPid]:Save()
            Players[targetPid]:LoadAllies()
            if type(Players[pid].SyncQuestStateWithOnlineAllies) == "function" then
                Players[pid]:SyncQuestStateWithOnlineAllies()
            end
        else
...
defaultCommands.leaveTeam = function(pid, cmd)
    ...
    elseif logicHandler.CheckPlayerValidity(pid, cmd[2]) then
        ...
        if containsPlayerReference(Players[pid].data.alliedPlayers, Players[targetPid]) then
            ...
            removePlayerReferences(Players[pid].data.alliedPlayers, Players[targetPid])
            removePlayerReferences(Players[targetPid].data.alliedPlayers, Players[pid])
            Players[pid]:Save()
            Players[pid]:LoadAllies()
            Players[targetPid]:Save()
            Players[targetPid]:LoadAllies()
...
defaultCommands.localMessage = function(pid, cmd)
    ...
    sendCellChatMessage(pid, cellDescription, message)
end
...
defaultCommands.ic = function(pid, cmd)
    if cmd[2] == nil then
        tes3mp.SendMessage(pid, "You cannot send a blank message.\n", false)
        return
    end
    ...
    local cellDescription = Players[pid].data.location.cell
    local message = color.White .. "[IC] " .. logicHandler.GetChatName(pid) .. " to local area: "
    message = message .. tableHelper.concatenateFromIndex(cmd, 2) .. "\n"

    sendCellChatMessage(pid, cellDescription, message)
end

customCommandHooks.registerCommand("ic", defaultCommands.ic)
