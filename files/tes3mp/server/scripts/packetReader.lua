...
local function AddPlayerIdentity(targetTable, player)
    if targetTable == nil or player == nil then
        return
    end

    targetTable.playerName = player.accountName
    targetTable.accountName = player.accountName
    targetTable.characterName = player.name

    if type(player.GetCharacterStorageKey) == "function" then
        targetTable.playerKey = player:GetCharacterStorageKey()
    end
end
...
packetReader.GetPlayerPacketTables = function(pid, packetType)
    ...
    if packetType == "PlayerBaseInfo" then
        packetTable.character = {
            name = tes3mp.GetName(pid),
            race = tes3mp.GetRace(pid),
            head = tes3mp.GetHead(pid),
            hair = tes3mp.GetHair(pid),
            gender = tes3mp.GetIsMale(pid),
            modelOverride = tes3mp.GetModel(pid),
            birthsign = tes3mp.GetBirthsign(pid)
        }
    elseif packetType == "PlayerClass" then
        packetTable.character = {}
    ...
    elseif packetType == "PlayerCellChange" then
        packetTable.location = {
            ...
            cellChangeReason = tes3mp.GetCellChangeReason(pid)
        }
    ...
    elseif packetType == "PlayerSpellsActive" then
        ...
        for changesIndex = 0, tes3mp.GetSpellsActiveChangesSize(pid) - 1 do
            ...
            if spellInstance.hasPlayerCaster == true then
                ...
                AddPlayerIdentity(spellInstance.caster, Players[casterPid])
            else
    ...
    elseif packetType == "PlayerJournal" then
        ...
        for changesIndex = 0, tes3mp.GetJournalChangesSize(pid) - 1 do
            ...
            elseif journalItem.type == enumerations.journal.FINISHED then
                journalItem.isFinished = tes3mp.GetJournalItemFinished(pid, changesIndex)
            end
    ...
    for packetIndex = 0, actorListSize - 1 do
        ...
        elseif packetType == "ActorSpellsActive" then
            ...
            for spellIndex = 0, spellsActiveChangesSize - 1 do
                ...
                if spellInstance.hasPlayerCaster == true then
                    ...
                    AddPlayerIdentity(spellInstance.caster, Players[spellInstance.caster.pid])
                else
        ...
        elseif packetType == "ActorDeath" then
            ...
            if doesActorHavePlayerKiller then
                ...
                AddPlayerIdentity(actor.killer, Players[actor.killer.pid])
            else
    ...
    for packetIndex = 0, objectListSize - 1 do
        ...
        if tableHelper.containsValue({"ObjectActivate", "ObjectHit", "ObjectSound", "ConsoleCommand"}, packetType) then
                ...
                elseif packetType == "ObjectSpawn" then
                    ...
                    if summonState == true then
                        ...
                        if object.summon.hasPlayerSummoner == true then
                            ...
                            AddPlayerIdentity(object.summon.summoner, Players[object.summon.summoner.pid])
                        else
            ...
            elseif packetType == "DoorDestination" then
                object.teleportState = tes3mp.GetObjectDoorTeleportState(packetIndex)

                if object.teleportState == true then
                    object.doorDestination = {
                        cell = tes3mp.GetObjectDoorDestinationCell(packetIndex),
                        posX = tes3mp.GetObjectDoorDestinationPosX(packetIndex),
                        posY = tes3mp.GetObjectDoorDestinationPosY(packetIndex),
                        posZ = tes3mp.GetObjectDoorDestinationPosZ(packetIndex),
                        rotX = tes3mp.GetObjectDoorDestinationRotX(packetIndex),
                        rotZ = tes3mp.GetObjectDoorDestinationRotZ(packetIndex)
                    }
                end
