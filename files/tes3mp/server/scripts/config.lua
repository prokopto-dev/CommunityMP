...
config.chatWindowInstructions = color.White .. "Press " .. color.Yellow .. "Y" .. color.White ..
    " to chat and " .. color.Yellow .. "Tab" .. color.White .. " to switch chat tabs. " ..
    "Text in All or Global is OOC; text in Local is IC. Type " .. color.Yellow ..
    "/help" .. color.White .. " for commands.\n"
...
-- Set this to true for the old server-wide cooperative quest log behavior.
config.shareJournal = false

-- Whether accepted journal changes should also be shared with the player's online allies.
-- This is ignored when shareJournal is enabled, because the world journal already syncs everyone.
...
-- Set this to true for the old server-wide cooperative topic discovery behavior.
...
-- CommunityMP defaults to the exterior Seyda Neen office exit so players begin
-- together in the world after chargen instead of inside the single-player office.
...
-- Vanilla chargen tutorial items that should not persist in multiplayer inventories
...
config.noninstancedSpawn = {
    cellDescription = "-2, -9",
    position = {-10963.0, -71344.0, 346.0},
    rotation = {0, 2.76},
    text = nil
}
...
-- Whether a newly arriving player with lower ping can automatically take over cell authority.
-- Keeping this disabled makes NPC AI, movement and combat ownership stable until the current
...
-- Containers listed here keep per-character contents instead of shared world contents.
-- This is used for quest-critical vanilla containers that must remain available to
-- every character while normal world containers stay shared.
...
-- Containers touched through these quest-critical items are also treated as
-- per-character, covering vanilla quest loot that can appear in otherwise
-- shared barrels or other ordinary containers.
config.playerScopedContainerItemRefIds = {
    ring_keley = true,
    ring_healing_01 = true
}
...
config.cellPacketTypes = { "delete", "place", "spawn", "move", "rotate", "lock", "trap", "scale", "state", "miscellaneous",
    "doorDestination", "doorState", "clientScriptLocal", "container", "equipment", "ai", "death", "actorList", "position",
...
-- defined in data/saves/server/config/data-files.xml. CommunityMP can import the old root
-- data-file JSON once for existing server folders that have not created the XML registry yet.
...
config.playerKeyOrder = { "login", "characters", "name", "passwordHash", "passwordSalt", "timestamps", "settings",
    "character", "customClass", "location", "stats", "fame", "shapeshift", "death", "attributes",
    ...
    "playerScopedContainers", "ipAddresses", "customVariables", "admin", "difficulty", "enforcedLogLevel", "physicsFramerate",
...
config.cellKeyOrder = { "packets", "entry", "lastVisit", "recordLinks", "objectData", "refId", "count",
    ...
    "cellChangeTo", "container", "death", "delete", "doorDestination", "doorState", "equipment", "inventory", "lock",
    "move", "place", "position", "rotate", "scale", "spawn", "state", "statsDynamic", "trap" }
