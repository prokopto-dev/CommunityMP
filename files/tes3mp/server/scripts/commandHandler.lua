    ...
    elseif (cmd[1] == "setauthority" or cmd[1] == "setauth") and moderator and #cmd > 2 then
        if logicHandler.CheckPlayerValidity(pid, cmd[2]) then
            ...
            if logicHandler.IsCellLoaded(cellDescription) == true then
                ...
                if not logicHandler.SetCellAuthority(targetPid, cellDescription) then
                    tes3mp.SendMessage(pid, "Player " .. targetPid .. " does not have cell \"" ..
                        cellDescription .. "\" loaded, so they cannot be cell authority.\n", false)
                end
            else
