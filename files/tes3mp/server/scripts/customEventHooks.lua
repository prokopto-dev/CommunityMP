...
customEventHooks.eventAliases = {
    onPlayerJournal = "OnPlayerJournal"
}

function customEventHooks.getEventName(event)
    return customEventHooks.eventAliases[event] or event
end
...
function customEventHooks.registerValidator(event, callback)
    event = customEventHooks.getEventName(event)
