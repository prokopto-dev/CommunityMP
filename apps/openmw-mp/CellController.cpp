...
const CellController::TContainer& CellController::getCells() const
{
    return cells;
}
    ...
    for (auto it = cells.begin(); it != cells.end();)
    {
        if (*it != nullptr && *it == cell)
        {
            ...
            for (auto&& playerEntry : *Players::getPlayers())
            {
                ...
                loadedCells->erase(std::remove(loadedCells->begin(), loadedCells->end(), cell), loadedCells->end());
            }
...
void CellController::deletePlayer(Player *player)
{
    ...
    for (; it != endIter; ++it)
    {
        ...
        if (!c->hasPlayers() && !c->hasSimulationInterest())
