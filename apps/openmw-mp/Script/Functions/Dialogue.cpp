...
unsigned int DialogueFunctions::GetTopicChangesSize(unsigned short pid) noexcept
{
    ...
    return static_cast<unsigned int>(player->topicChanges.size());
}
...
void DialogueFunctions::SetTopicChangesAreLoad(unsigned short pid, bool value) noexcept
{
    Player *player;
    GET_PLAYER(pid, player, );

    player->topicChangesAreLoad = value;
}
