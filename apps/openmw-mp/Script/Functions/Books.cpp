...
void BookFunctions::ClearBookChanges(unsigned short pid) noexcept
{
    ...
    player->bookChangesAreLoad = false;
}
...
unsigned int BookFunctions::GetBookChangesSize(unsigned short pid) noexcept
{
    ...
    return static_cast<unsigned int>(player->bookChanges.size());
}
...
void BookFunctions::SetBookChangesAreLoad(unsigned short pid, bool value) noexcept
{
    Player *player;
    GET_PLAYER(pid, player, );

    player->bookChangesAreLoad = value;
}
