...
static std::string tempStatRefId;

namespace
{
    std::string getRefIdString(const ESM::RefId& refId)
    {
        if (refId.empty())
            return {};
        return refId.getRefIdString();
    }

    const char* getRefIdCString(const ESM::RefId& refId)
    {
        tempStatRefId = getRefIdString(refId);
        return tempStatRefId.c_str();
    }
}
...
const char *StatsFunctions::GetAttributeName(unsigned short attributeId) noexcept
{
    ...
    tempStatRefId = ESM::Attribute::indexToRefId(attributeId).getRefIdString();
    return tempStatRefId.c_str();
}
...
const char *StatsFunctions::GetSkillName(unsigned short skillId) noexcept
{
    ...
    tempStatRefId = ESM::Skill::indexToRefId(skillId).getRefIdString();
    return tempStatRefId.c_str();
}
...
const char *StatsFunctions::GetRace(unsigned short pid) noexcept
{
    ...
    return getRefIdCString(player->npc.mRace);
}
...
const char *StatsFunctions::GetHead(unsigned short pid) noexcept
{
    ...
    return getRefIdCString(player->npc.mHead);
}
...
const char *StatsFunctions::GetHairstyle(unsigned short pid) noexcept
{
    ...
    return getRefIdCString(player->npc.mHair);
}
...
int StatsFunctions::GetAttributeBase(unsigned short pid, unsigned short attributeId) noexcept
{
    ...
    return static_cast<int>(player->creatureStats.mAttributes[attributeId].mBase);
}
...
int StatsFunctions::GetAttributeModifier(unsigned short pid, unsigned short attributeId) noexcept
{
    ...
    return static_cast<int>(player->creatureStats.mAttributes[attributeId].mMod);
}
...
int StatsFunctions::GetSkillBase(unsigned short pid, unsigned short skillId) noexcept
{
    ...
    return static_cast<int>(player->npcStats.mSkills[skillId].mBase);
}
...
int StatsFunctions::GetSkillModifier(unsigned short pid, unsigned short skillId) noexcept
{
    ...
    return static_cast<int>(player->npcStats.mSkills[skillId].mMod);
}
...
void StatsFunctions::SetRace(unsigned short pid, const char *race) noexcept
{
    ...
    LOG_MESSAGE_SIMPLE(TimedLog::LOG_VERBOSE, "Setting race for %s: %s -> %s", player->npc.mName.c_str(),
                       getRefIdString(player->npc.mRace).c_str(), race);
...
void StatsFunctions::SetHealthBase(unsigned short pid, double value) noexcept
{
    ...
    player->creatureStats.mDynamic[0].mBase = static_cast<float>(value);
...
void StatsFunctions::SetHealthCurrent(unsigned short pid, double value) noexcept
{
    ...
    player->creatureStats.mDynamic[0].mCurrent = static_cast<float>(value);
...
void StatsFunctions::SetMagickaBase(unsigned short pid, double value) noexcept
{
    ...
    player->creatureStats.mDynamic[1].mBase = static_cast<float>(value);
...
void StatsFunctions::SetMagickaCurrent(unsigned short pid, double value) noexcept
{
    ...
    player->creatureStats.mDynamic[1].mCurrent = static_cast<float>(value);
...
void StatsFunctions::SetFatigueBase(unsigned short pid, double value) noexcept
{
    ...
    player->creatureStats.mDynamic[2].mBase = static_cast<float>(value);
...
void StatsFunctions::SetFatigueCurrent(unsigned short pid, double value) noexcept
{
    ...
    player->creatureStats.mDynamic[2].mCurrent = static_cast<float>(value);
...
void StatsFunctions::SetAttributeBase(unsigned short pid, unsigned short attributeId, int value) noexcept
{
    ...
    player->creatureStats.mAttributes[attributeId].mBase = static_cast<float>(value);

    if (!Utils::vectorContains(player->attributeIndexChanges, attributeId))
        player->attributeIndexChanges.push_back(static_cast<unsigned char>(attributeId));
}
...
void StatsFunctions::SetAttributeDamage(unsigned short pid, unsigned short attributeId, double value) noexcept
{
    ...
    player->creatureStats.mAttributes[attributeId].mDamage = static_cast<float>(value);
...
void StatsFunctions::SetSkillBase(unsigned short pid, unsigned short skillId, int value) noexcept
{
    ...
    player->npcStats.mSkills[skillId].mBase = static_cast<float>(value);

    if (!Utils::vectorContains(player->skillIndexChanges, skillId))
        player->skillIndexChanges.push_back(static_cast<unsigned char>(skillId));
}
...
void StatsFunctions::SetSkillDamage(unsigned short pid, unsigned short skillId, double value) noexcept
{
    ...
    player->npcStats.mSkills[skillId].mDamage = static_cast<float>(value);
...
void StatsFunctions::SetSkillProgress(unsigned short pid, unsigned short skillId, double value) noexcept
{
    ...
    player->npcStats.mSkills[skillId].mProgress = static_cast<float>(value);
...
void StatsFunctions::SetCharGenStage(unsigned short pid, int currentStage, int endStage) noexcept
{
    ...
    mwmp::PlayerPacket *packet = mwmp::ServerNetworking::get().getPlayerPacketController()->GetPacket(ID_PLAYER_CHARGEN);
...
void StatsFunctions::SendBaseInfo(unsigned short pid) noexcept
{
    ...
    mwmp::PlayerPacket *packet = mwmp::ServerNetworking::get().getPlayerPacketController()->GetPacket(ID_PLAYER_BASEINFO);
    packet->setPlayer(player);
...
void StatsFunctions::SendStatsDynamic(unsigned short pid) noexcept
{
    ...
    mwmp::PlayerPacket *packet = mwmp::ServerNetworking::get().getPlayerPacketController()->GetPacket(ID_PLAYER_STATS_DYNAMIC);
    packet->setPlayer(player);
...
void StatsFunctions::SendAttributes(unsigned short pid) noexcept
{
    ...
    mwmp::PlayerPacket *packet = mwmp::ServerNetworking::get().getPlayerPacketController()->GetPacket(ID_PLAYER_ATTRIBUTE);
    packet->setPlayer(player);
...
void StatsFunctions::SendSkills(unsigned short pid) noexcept
{
    ...
    mwmp::PlayerPacket *packet = mwmp::ServerNetworking::get().getPlayerPacketController()->GetPacket(ID_PLAYER_SKILL);
    packet->setPlayer(player);
...
void StatsFunctions::SendLevel(unsigned short pid) noexcept
{
    ...
    mwmp::PlayerPacket *packet = mwmp::ServerNetworking::get().getPlayerPacketController()->GetPacket(ID_PLAYER_LEVEL);
    packet->setPlayer(player);
...
void StatsFunctions::SendBounty(unsigned short pid) noexcept
{
    ...
    mwmp::PlayerPacket *packet = mwmp::ServerNetworking::get().getPlayerPacketController()->GetPacket(ID_PLAYER_BOUNTY);
    packet->setPlayer(player);
