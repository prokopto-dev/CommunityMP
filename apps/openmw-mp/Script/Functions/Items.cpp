...
unsigned int ItemFunctions::GetInventoryChangesSize(unsigned short pid) noexcept
{
    ...
    return static_cast<unsigned int>(player->inventoryChanges.items.size());
}
    ...
    int charge, double enchantmentCharge) noexcept
{
    ...
    player->equipmentItems[slot].enchantmentCharge = static_cast<float>(enchantmentCharge);
    ...
    double enchantmentCharge, const char* soul) noexcept
{
    ...
    item.count = count;
    item.charge = charge;
    item.enchantmentCharge = static_cast<float>(enchantmentCharge);
    item.soul = soul;
