...
{
    return static_cast<unsigned int>(readWorldstate->killChanges.size());
}

unsigned int WorldstateFunctions::GetMapChangesSize() noexcept
{
    return static_cast<unsigned int>(readWorldstate->mapTiles.size());
}

unsigned int WorldstateFunctions::GetClientGlobalsSize() noexcept
{
    return static_cast<unsigned int>(readWorldstate->clientGlobals.size());
}
...
{
    writeWorldstate.weather.transitionFactor = static_cast<float>(transitionFactor);
}

void WorldstateFunctions::SetHour(double hour) noexcept
{
    writeWorldstate.time.hour = static_cast<float>(hour);
}
...
{
    writeWorldstate.time.timeScale = static_cast<float>(timeScale);
}
...
void WorldstateFunctions::AddClientGlobalFloat(const char* id, double floatValue) noexcept
{
    ...
    clientVariable.floatValue = static_cast<float>(floatValue);
