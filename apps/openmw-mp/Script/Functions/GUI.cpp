...
void GUIFunctions::ListBoxWithMetadata(unsigned short pid, int id, const char *label, const char *items,
    const char *metadata)
{
    Player *player;
    GET_PLAYER(pid, player,);
...
unsigned int GUIFunctions::GetQuickKeyChangesSize(unsigned short pid) noexcept
{
    ...
    return static_cast<unsigned int>(player->quickKeyChanges.size());
}
