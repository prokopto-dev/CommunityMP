...
const char* MiscellaneousFunctions::GenerateRandomString(unsigned int length) noexcept
{
    ...
    std::uniform_int_distribution<std::mt19937::result_type> distribution(
        0, static_cast<std::mt19937::result_type>(characters.size() - 1));
...
const char* MiscellaneousFunctions::GetHandshakePasswordHash(unsigned short pid) noexcept
{
    Player *player;
    GET_PLAYER(pid, player, "");

    return player->getLoginPasswordHash().c_str();
}

void MiscellaneousFunctions::ClearHandshakePasswordHash(unsigned short pid) noexcept
{
    Player *player;
    GET_PLAYER(pid, player,);

    player->clearLoginPasswordHash();
}
