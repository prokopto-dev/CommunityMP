...
#define GUIAPI \
    ...
    {"ListBoxWithMetadata",        GUIFunctions::ListBoxWithMetadata},\
    \
...
public:
    ...
    */
    ...
    /**
    * \brief Display a listbox with client-only metadata aligned to the visible rows.
    *
    * Existing scripts should keep using ListBox. This helper is for menus that need
    * extra client presentation data without exposing it in the visible item text.
    *
    * \param pid The player ID for whom the listbox should appear.
    * \param id The numerical ID of the listbox.
    * \param label The text at the top of the listbox.
    * \parm items The visible items in the listbox, separated by newlines.
    * \parm metadata The hidden metadata rows, separated by newlines.
    * \return void
    */
    static void ListBoxWithMetadata(unsigned short pid, int id, const char *label, const char *items,
        const char *metadata);
