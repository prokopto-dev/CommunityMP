...
template<> struct TypeChar<mwmp::PacketNetworkId**, sizeof(mwmp::PacketNetworkId**)> { enum { value = 'n' }; };
...
template<> struct CharType<'n'> { typedef mwmp::PacketNetworkId** type; };
...
struct ScriptIdentity
{
    ...
    constexpr bool matches(const char* signature, const unsigned int N = 0) const
    {
        return N < numargs ? this->types[N] == signature[N] && matches(signature, N + 1) : this->types[N] == signature[N];
    }
...
struct ScriptFunctionSignature : public ScriptIdentity
{
    template<typename R, typename... Types>
    constexpr ScriptFunctionSignature(Function<R, Types...> addr) : ScriptIdentity(addr) {}
};

struct ScriptFunctionSignatureData
{
    const char* name;
    const ScriptFunctionSignature func;

    constexpr ScriptFunctionSignatureData(const char* name, ScriptFunctionSignature func) : name(name), func(func) {}
};
