...
ESM::Cell Utils::getCellFromDescription(std::string cellDescription)
{
    ...
    static std::regex exteriorCellPattern("^(-?\\d+),\\s*(-?\\d+)$");
    static std::regex namedExteriorCellPattern("^.+\\((-?\\d+),\\s*(-?\\d+)\\)$");
    std::smatch baseMatch;

    if (std::regex_match(cellDescription, baseMatch, exteriorCellPattern)
        || std::regex_match(cellDescription, baseMatch, namedExteriorCellPattern))
    {
