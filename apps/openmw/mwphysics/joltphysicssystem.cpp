...
namespace MWPhysics
{
        ...
        const MWWorld::Ptr& ptr, VFS::Path::NormalizedView mesh, osg::Quat rotation, int /*collisionType*/)
    {
        ...
        if (std::isfinite(scale) && std::abs(scale - 1.0f) > 0.0001f)
            joltShape = new JPH::ScaledShape(joltShape, JPH::Vec3(scale, scale, scale));
        ...
        const osg::Quat* initialRotation)
    {
        ...
        mDynamicBodies.emplace(ptr.mRef,
            DynamicBody{ id, ptr, pivotLift, navmeshShape, /*mInNavmesh*/ false,
                /*mWasActive*/ true, /*mHoldTimer*/ 0.0f, pos.asVec3(), q, true });
        mBodyOwners.emplace(id.GetIndexAndSequenceNumber(), ptr);
    }
    ...
    void JoltPhysicsSystem::updatePtr(const MWWorld::Ptr& old, const MWWorld::Ptr& updated)
    {
        ...
        if (auto it = mDynamicBodies.find(old.mRef); it != mDynamicBodies.end())
        {
            it->second.mPtr = updated;
            mBodyOwners[it->second.mBodyId.GetIndexAndSequenceNumber()] = updated;
        }
    ...
    void JoltPhysicsSystem::updateRotation(const MWWorld::Ptr& ptr, osg::Quat rotate)
    {
        ...
        {
            const ESM::Position& pos = ptr.getRefData().getPosition();
            const osg::Vec3f visualPos(pos.pos[0], pos.pos[1], pos.pos[2]);
            const osg::Vec3f rotatedLift = rotate * dit->second.mPivotLift;
            auto& bi = mJoltSystem->GetBodyInterface();
            bi.SetPositionAndRotation(dit->second.mBodyId,
                JPH::RVec3(visualPos.x() + rotatedLift.x(), visualPos.y() + rotatedLift.y(),
                    visualPos.z() + rotatedLift.z()),
                jrot, JPH::EActivation::Activate);
            dit->second.mLastPosition = visualPos;
            dit->second.mLastRotation = rotate;
            dit->second.mActive = true;
            return;
        }
        if (auto it = mObjectBodies.find(ptr.mRef); it != mObjectBodies.end())
        {
    ...
    void JoltPhysicsSystem::updatePosition(const MWWorld::Ptr& ptr)
    {
        ...
        {
            auto& bi = mJoltSystem->GetBodyInterface();
            const JPH::Quat jrot = bi.GetRotation(dit->second.mBodyId);
            const osg::Quat osgRot(jrot.GetX(), jrot.GetY(), jrot.GetZ(), jrot.GetW());
            const osg::Vec3f visualPos(pos.pos[0], pos.pos[1], pos.pos[2]);
            const osg::Vec3f rotatedLift = osgRot * dit->second.mPivotLift;
            bi.SetPosition(dit->second.mBodyId,
                JPH::RVec3(visualPos.x() + rotatedLift.x(), visualPos.y() + rotatedLift.y(),
                    visualPos.z() + rotatedLift.z()),
                JPH::EActivation::Activate);
            dit->second.mLastPosition = visualPos;
            dit->second.mLastRotation = osgRot;
            dit->second.mActive = true;
        }
        else if (auto it = mObjectBodies.find(ptr.mRef); it != mObjectBodies.end())
        {
        ...
        float dt, bool skipSimulation, osg::Timer_t /*frameStart*/, unsigned int frameNumber, osg::Stats& /*stats*/)
    {
        ...
        if (!mDynamicBodies.empty())
        {
            ...
            for (auto& [_, dyn] : mDynamicBodies)
            {
                ...
                // Rotation stays on the live Jolt body and the scene node.
                // The fork persisted it through RefData::DynamicBodyState,
                // but this branch does not carry that save-format extension.
                ...
                dyn.mLastPosition = visualPos;
                dyn.mLastRotation = osgRot;
                dyn.mActive = active;
    ...
    osg::Vec3f JoltPhysicsSystem::getInertialForce(const MWWorld::ConstPtr& ptr) const
    {
        const auto it = mActors.find(ptr.mRef);
        return it != mActors.end() ? osg::Vec3f(0.f, 0.f, it->second->getInertiaZ()) : osg::Vec3f();
    }

    void JoltPhysicsSystem::setInertialForce(const MWWorld::Ptr& ptr, const osg::Vec3f& force)
    {
        const auto it = mActors.find(ptr.mRef);
        if (it != mActors.end())
            it->second->setInertiaZ(force.z());
    }

    void JoltPhysicsSystem::forceActorFall(const MWWorld::Ptr& ptr)
    {
        const auto it = mActors.find(ptr.mRef);
        if (it == mActors.end())
            return;

        const float gravityKick = -Constants::GravityConst * Constants::UnitsPerMeter * 0.1f;
        it->second->setInertiaZ(std::min(it->second->getInertiaZ(), gravityKick));
        it->second->forceFall();
    }
    ...
    std::vector<IPhysicsBackend::DynamicBodySnapshot> JoltPhysicsSystem::getDynamicBodySnapshots() const
    {
        std::vector<DynamicBodySnapshot> out;
        out.reserve(mDynamicBodies.size());
        for (const auto& [_, dyn] : mDynamicBodies)
        {
            if (dyn.mPtr.isEmpty())
                continue;

            DynamicBodySnapshot snapshot;
            snapshot.mPtr = dyn.mPtr;
            snapshot.mPosition = dyn.mLastPosition;
            snapshot.mRotation = dyn.mLastRotation;
            snapshot.mActive = dyn.mActive;
    ...
    bool JoltPhysicsSystem::setPhysicsFrameRate(float framesPerSecond)
    {
