...
namespace MWPhysics
{
    namespace
    {
        ...
        JPH::RefConst<JPH::Shape> rotateYAlignedShapeToBulletAxis(
            JPH::RefConst<JPH::Shape> shape, int bulletUpAxis)
        {
            if (!shape || bulletUpAxis == 1)
                return shape;

            JPH::Quat rotation = JPH::Quat::sIdentity();
            if (bulletUpAxis == 0)
                rotation = JPH::Quat::sRotation(JPH::Vec3::sAxisZ(), -0.5f * JPH::JPH_PI);
            else if (bulletUpAxis == 2)
                rotation = JPH::Quat::sRotation(JPH::Vec3::sAxisX(), 0.5f * JPH::JPH_PI);
            else
                return shape;

            JPH::RotatedTranslatedShapeSettings settings(JPH::Vec3::sZero(), rotation, shape.GetPtr());
            const auto result = settings.Create();
            if (result.HasError())
            {
                Log(Debug::Warning) << "Jolt RotatedTranslatedShape rejected: " << result.GetError();
                return nullptr;
            }
            return result.Get();
        }
        ...
        JPH::RefConst<JPH::Shape> convertCapsule(const btCapsuleShape* shape)
        {
            ...
            return rotateYAlignedShapeToBulletAxis(result.Get(), shape->getUpAxis());
        }
        ...
        JPH::RefConst<JPH::Shape> convertCylinder(const btCylinderShape* shape)
        {
            ...
            const int axis = shape->getUpAxis();
            const float halfHeight = half[axis];
            const float radius = std::max(half[(axis + 1) % 3], half[(axis + 2) % 3]);
            ...
            return rotateYAlignedShapeToBulletAxis(result.Get(), axis);
        }
    }
    ...
    namespace
    {
        ...
        void collectVertices(const btCollisionShape* shape, std::vector<JPH::Vec3>& out)
        {
            ...
            {
                const auto* poly = static_cast<const btPolyhedralConvexShape*>(shape);
                const int n = poly->getNumVertices();
                out.reserve(out.size() + static_cast<size_t>(std::max(0, n)));
                for (int i = 0; i < n; ++i)
                {
                    btVector3 vertex;
                    poly->getVertex(i, vertex);
                    out.emplace_back(vertex.x(), vertex.y(), vertex.z());
                }
                return;
            }
            if (shape->isConcave())
    ...
    JPH::RefConst<JPH::Shape> convertBulletShape(const btCollisionShape* shape)
    {
        ...
        switch (t)
        {
            ...
            default:
                if (shape->isConvex())
                {
                    JPH::RefConst<JPH::Shape> fallback = extractConvexHull(shape);
                    if (fallback)
                    {
                        Log(Debug::Warning) << "Jolt shape converter: approximated unsupported convex Bullet shape type "
                                            << t << " (" << shape->getName() << ") with a convex hull";
                        return fallback;
                    }
                }

                // Height fields are added through JoltPhysicsSystem::addHeightField.
