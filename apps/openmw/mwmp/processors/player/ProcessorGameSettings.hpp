    ...
    public:
        ...
        virtual void Do(PlayerPacket &packet, BasePlayer *player)
        {
            ...
            if (isLocal())
            {
                ...
                if (MWBase::Environment::get().getWorld()->setPhysicsFrameRate(player->physicsFramerate)
                    && player->physicsFramerate != 60.0f)
                {
                    LOG_APPEND(TimedLog::LOG_INFO, "- server is enforcing physics framerate %f",
                        player->physicsFramerate);
                }
