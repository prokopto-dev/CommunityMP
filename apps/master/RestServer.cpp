...
void RestServer::start()
{
    ...
    httpServer.resource[ServersRegex]["GET"] = [this](auto response, auto request) {
        if (request->path_match[1].length() > 0)
        {
            try
            {
                ...
                queryToStringStream(ss, "server", serverMap->at(makePacketAddress(addr.c_str(), port)));
                ss << "}";
        ...
        else
        {
            ...
            //if (updatedCache)
            {
                ...
                {
                    queryToStringStream(ss, packetAddressToString(query->first, true, ':'), query->second);
    ...
    httpServer.resource[ServersRegex]["POST"] = [this](auto response, auto request) {
        try
        {
            ...
            serverMap->insert({makePacketAddress(request->remote_endpoint_address.c_str(), port), server});
            updatedCache = true;
    ...
    httpServer.resource[ServersRegex]["PUT"] = [this](auto response, auto request) {
        ...
        auto query = serverMap->find(makePacketAddress(request->remote_endpoint_address.c_str(), port));
