...
namespace SimpleWeb
{
    ...
    protected:
        ...
        get_timeout_timer(const std::shared_ptr<socket_type> &socket, size_t seconds)
        {
            if (seconds == 0)
                return nullptr;

            auto timer = std::make_shared<boost::asio::steady_timer>(*io_service);
            timer->expires_after(std::chrono::seconds(static_cast<std::chrono::seconds::rep>(seconds)));
            timer->async_wait([socket](const boost::system::error_code &ec)
                              {
                                  if (!ec)
                                  {
                                      boost::system::error_code close_error;
                                      socket->lowest_layer().shutdown(boost::asio::ip::tcp::socket::shutdown_both, close_error);
                     ...
                     size_t bytes_transferred)
            {
                ...
                if (!ec)
                {
                    ...
                    if (it != request->header.end())
                    {
                        ...
                        {
                            //Set timeout on the following boost::asio::async-read or write function
                            auto content_timer = this->get_timeout_timer(socket,
                                                                 config.timeout_content);
                            boost::asio::async_read(*socket, request->streambuf,
                                                    boost::asio::transfer_exactly(
                                                            content_length -
                                                            num_additional_bytes),
                                                    [this, socket, request, content_timer]
                                                            (const boost::system::error_code &ec,
                                                             size_t /*bytes_transferred*/)
                                                    {
                                                        if (content_timer)
                                                            content_timer->cancel();
                                                        if (!ec)
