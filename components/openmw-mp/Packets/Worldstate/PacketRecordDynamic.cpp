...
void PacketRecordDynamic::Packet(PacketStream *newBitstream, bool send)
{
    WorldstatePacket::Packet(newBitstream, send);

    if (!RW(worldstate->recordsType, send))
        return;
    ...
    if (!RW(worldstate->recordsCount, send))
        return;
    ...
    else if (worldstate->recordsType == mwmp::RECORD_TYPE::POTION)
    {
        for (auto &&record : worldstate->potionRecords)
        {
            ...
            int32_t autoCalc = 0;
            if (send)
                autoCalc = (recordData.mData.mFlags & ESM::Potion::Autocalc) != 0;
            RW(autoCalc, send);
            if (!send)
            {
                if (autoCalc)
    ...
    else if (worldstate->recordsType == mwmp::RECORD_TYPE::MISCELLANEOUS)
    {
        for (auto &&record : worldstate->miscellaneousRecords)
        {
            ...
            int32_t isKey = 0;
            if (send)
                isKey = (recordData.mData.mFlags & ESM::Miscellaneous::Key) != 0;
            RW(isKey, send);
            if (!send)
            {
                if (isKey)
...
void PacketRecordDynamic::ProcessEffects(ESM::EffectList &effectList, bool send)
{
    ...
    for (auto &&effect : effectList.mList)
    {
        ...
        if (send)
        {
            effectId = ESM::MagicEffect::refIdToIndex(effectData.mEffectID);
            attribute = ESM::Attribute::refIdToIndex(effectData.mAttribute);
            skill = ESM::Skill::refIdToIndex(effectData.mSkill);
        }

        RW(effectId, send);
        RW(attribute, send);
        RW(skill, send);
        RW(effectData.mRange, send);
        RW(effectData.mArea, send);
        RW(effectData.mDuration, send);
        RW(effectData.mMagnMax, send);
        RW(effectData.mMagnMin, send);

        if (!send)
        {
            effectData.mEffectID = ESM::MagicEffect::indexToRefId(effectId);
            effectData.mAttribute = ESM::Attribute::indexToRefId(attribute);
            effectData.mSkill = ESM::Skill::indexToRefId(skill);
        }
    }
