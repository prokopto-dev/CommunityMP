...
namespace
{
    ...
    osg::Vec3f toOsg(const btVector3& value)
    {
        return osg::Vec3f(static_cast<float>(value.x()), static_cast<float>(value.y()), static_cast<float>(value.z()));
    }

    std::unique_ptr<btCollisionShape> bakeTriangleMeshTransform(
        const btBvhTriangleMeshShape& shape, const osg::Matrixf& transform)
    {
        auto triangles = std::make_unique<btTriangleMesh>(false);

        auto callback = BulletHelpers::makeProcessTriangleCallback([&](btVector3* triangle, int, int) {
            triangles->addTriangle(Misc::Convert::toBullet(transform.preMult(toOsg(triangle[0]))),
                Misc::Convert::toBullet(transform.preMult(toOsg(triangle[1]))),
                Misc::Convert::toBullet(transform.preMult(toOsg(triangle[2]))));
        });
        ...
        auto result = std::make_unique<Resource::TriangleMeshShape>(triangles.get(), true);
        std::ignore = triangles.release();
        return result;
    }
...
namespace NifBullet
{
    ...
    void BulletNifLoader::handleRoot(Nif::FileView nif, const Nif::NiAVObject& node, HandleNodeArgs args)
    {
        ...
        else
        {
            ...
            args.mUseStringExtraData = true;

            // Check for root extra data. Nested extra data is handled while traversing nodes.
            args.mRoot = true;
            handleStringExtraData(node, args);
    ...
    void BulletNifLoader::handleNode(const Nif::NiAVObject& node, const Nif::Parent* parent, HandleNodeArgs args)
    {
        ...
        if (args.mUseStringExtraData && !args.mRoot)
            handleStringExtraData(node, args);

        if (args.mNoCollision)
            return;
        ...
        if (const Nif::NiNode* ninode = dynamic_cast<const Nif::NiNode*>(&node))
        {
            ...
            for (const auto& child : ninode->mChildren)
            {
                if (!child.empty())
                {
                    ...
                    HandleNodeArgs childArgs = args;
                    childArgs.mRoot = false;
                    handleNode(child.get(), &currentParent, childArgs);
                }
    ...
    void BulletNifLoader::handleStringExtraData(const Nif::NiAVObject& node, HandleNodeArgs& args)
    {
        if (!args.mRoot && dynamic_cast<const Nif::NiNode*>(&node) == nullptr)
            return;
        ...
        for (const auto& e : node.getExtraList())
        {
            ...
            {
                // String markers may contain important information affecting the entire subtree of this node.
        ...
        const Nif::NiGeometry& niGeometry, const Nif::Parent* nodeParent, HandleNodeArgs args)
    {
        ...
        const bool bakeTransform = !args.mAnimated && childShape->getShapeType() == TRIANGLE_MESH_SHAPE_PROXYTYPE
            && !transform.isIdentity();

        if (bakeTransform)
        {
            childShape = bakeTriangleMeshTransform(static_cast<const btBvhTriangleMeshShape&>(*childShape), transform);
            if (childShape == nullptr)
                return;
            transform = osg::Matrixf::identity();
        }
        else if (childShape->getShapeType() == TRIANGLE_MESH_SHAPE_PROXYTYPE)
        {
